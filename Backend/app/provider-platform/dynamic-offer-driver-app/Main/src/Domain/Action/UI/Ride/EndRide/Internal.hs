...
createDriverFee merchantId merchantOpCityId driverId rideFare currency newFareParams driverInfo booking serviceName = do
  ...
  where
    ...
    mkVendorFee driverFeeId now splitMethod vendorAmount = DVF.VendorFee {amount = HighPrecMoney (snd vendorAmount), driverFeeId = driverFeeId, vendorId = fst vendorAmount, splitMethod, vendorFeeProcessedAt = Nothing, createdAt = now, updatedAt = now}
