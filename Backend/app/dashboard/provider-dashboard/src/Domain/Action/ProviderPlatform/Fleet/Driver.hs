...
postDriverFleetDriversLocation :: (Kernel.Types.Id.ShortId DM.Merchant -> City.City -> ApiTokenInfo -> Common.DriversLocationReq -> Environment.Flow Common.DriverLocationResp)
postDriverFleetDriversLocation merchantShortId opCity apiTokenInfo req = do
  checkedMerchantId <- merchantCityAccessCheck merchantShortId apiTokenInfo.merchant.shortId opCity apiTokenInfo.city
  let memberPersonId = apiTokenInfo.personId.getId
  -- transaction <- SharedLogic.Transaction.buildTransaction (Domain.Types.Transaction.castEndpoint apiTokenInfo.userActionType) (Kernel.Prelude.Just DRIVER_OFFER_BPP_MANAGEMENT) (Kernel.Prelude.Just apiTokenInfo) Kernel.Prelude.Nothing Kernel.Prelude.Nothing (Kernel.Prelude.Just req)
  -- SharedLogic.Transaction.withTransactionStoring transaction $ (do error "Client call .driverDSL.postDriverFleetDriversLocation for separate helperApi couldn't be autogenerated. Logic yet to be decided" checkedMerchantId req)
  Client.callFleetAPI checkedMerchantId opCity (.driverDSL.postDriverFleetDriversLocation) memberPersonId req
